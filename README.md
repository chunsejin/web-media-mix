...
미디어믹스란 원래 광고업계의 용어로, 상품을 광고하기 위해 여러 매체를 조합함으로써, 각 매체의 약점을 보완하는 수법을 가리키는 말
- 전략적이고 효율적인 광고 운영을 위한 일종의 계획안
...
# 미디어 믹스의 활용
...
 osmu(원 소스 멀티 유즈): 한가지 소스로 여러가지 방면으로 사용하는 방법을 말함 
 - 하나의 캐릭터(= 원소스)를 이용하여 여러 방면(= 애니매이션, 캐릭터 굿즈, 웹툰, 테마파크 등)으로 사용하여 수익을 창출해 낸다.
